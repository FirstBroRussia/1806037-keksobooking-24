...
selectRoomCount.addEventListener('change', (evt) => {
  ...
  selectCapacity.value = selectedValue;
  const optionsSelectCapacity = selectCapacity.options;
  for (const index = 0; index < optionsSelectCapacity.length; index++) {
    if (index > selectedValue) {
      optionsSelectCapacity[index].setAttribute('disabled', true);
    } else {
      optionsSelectCapacity[index].setAttribute('disabled', false);
    }
  }
});
